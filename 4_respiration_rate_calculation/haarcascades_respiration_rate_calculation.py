        ...
        if previous_average_color is None:
            ...
            if len(signal_buffer) >= padlen:
                ...
                if len(peaks) >= 2:
                    ...
                    peak_times = np.array(
                        peaks) / video_capture.get(cv2.CAP_PROP_FPS)
                    ...
                    # Store heart rate in the buffer for moving average calculation
                    heart_rate_buffer.append(heart_rate)

                    # Calculate the moving average heart rate over a specified time window
                ...
                if len(signal_buffer) >= padlen:
                    ...
                    # Find dominant frequency associated with respiration
                    respiration_frequency_index = np.argmax(power_spectrum)
                    respiration_frequency = frequencies[
                        respiration_frequency_index
                    ]

                    # Estimate respiration rate as the dominant frequency
                    respiration_rate = respiration_frequency * 60
                    respiration_rate_buffer.append(respiration_rate)

                    # Calculate the moving average respiration rate over a specified time window
                    if len(respiration_rate_buffer) > respiration_rate_window:
                        respiration_rate_buffer = respiration_rate_buffer[
                            -respiration_rate_window:
                        ]
                        average_respiration_rate = np.mean(
                            respiration_rate_buffer)
                        print(
