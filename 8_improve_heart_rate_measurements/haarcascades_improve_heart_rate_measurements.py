...
def calculate_heart_rate(
    ...
    # Apply moving average smoothing to the signal buffer
    smoothed_signal = medfilt(signal_buffer)
    ...
    if len(peaks) >= 2:
        ...
        # Set a reasonable range for valid time differences between peaks
        min_time_diff = 0.2  # Minimum time difference in seconds
        max_time_diff = 1.0  # Maximum time difference in seconds

        # Calculate time differences between peaks
        time_diff = np.diff(peak_times)

        # Filter out peaks with time differences outside the valid range
        valid_peaks = [peaks[i] for i in range(
            len(time_diff)) if min_time_diff <= time_diff[i] <= max_time_diff]

        # Consider only the most recent peaks within the heart rate window
        recent_peaks = valid_peaks[-heart_rate_window:]

        # Calculate time differences between recent peaks
        recent_time_diff = np.diff(np.array(recent_peaks) / video_fps)

        if len(recent_time_diff) > 0:
            # Calculate heart rate as beats per minute based on recent peaks
            heart_rate = 60 / np.mean(recent_time_diff)
...
signal_buffer = []
padlen = 150  # Length of the signal buffer
